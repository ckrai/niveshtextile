...
//************************Blog Function*************//////////
    ...
    public function add_new_blog($title, $blog_tag, $blog_category, $author, $content, $slug, $image, $u_id, $u_name, $activity_msg, $activity_type)
    {
        ...
        $stmt = $this->con->prepare("INSERT INTO tech_blogs (blog_title, blog_tag, blog_category, blog_author, blog_image, blog_description, blog_date, blog_slug) VALUES (?,?,?,?,?,?,?,?)");
...
public function get_blog()
    {
        $stmt = $this->con->prepare("SELECT * FROM tech_blogs  ORDER BY blog_id DESC");
        $stmt->execute(); 
        return $stmt;
    }
    ...
    {
        $stmt = $this->con->prepare("SELECT * FROM tech_contact  ORDER BY con_id DESC");
        $stmt->execute(); 
        return $stmt;
    }
...
public function get_blog_edit($blog_id)
    {
        $stmt = $this->con->prepare("SELECT * FROM tech_blogs WHERE blog_id = :blog_id ORDER BY blog_id DESC");
...
public function get_blog_active()
    {
        $stmt = $this->con->prepare("SELECT * FROM tech_blogs WHERE blog_status=1  ORDER BY blog_id DESC");
        $stmt->execute(); 
        return $stmt;
    }
 // end 

      public function get_blog_front3($blog_id)
    {
         $stmt = $this->con->prepare("SELECT * FROM tech_blogs WHERE blog_status=1 AND blog_id!=$blog_id ORDER BY blog_id DESC LIMIT 0,3");
        $stmt->execute(); 
        return $stmt;
    }
...
public function update_blog_status($u_id, $u_name, $activity_msg, $activity_type, $blog_id, $status)
    {
       $stmt = $this->con->prepare("UPDATE tech_blogs SET blog_status =:status WHERE blog_id = :blog_id ");
...
public function delete_blog($u_id, $u_name, $activity_msg, $activity_type, $blog_id)
    {
        $stmt = $this->con->prepare("DELETE  FROM tech_blogs WHERE blog_id = :blog_id");
        $stmt->execute(array(':blog_id' => $blog_id));
...
//Update Blog....
    ...
    public function edit_blog($blogid,$title, $blog_tag, $blog_category, $author, $content, $slug, $image, $u_id, $u_name, $activity_msg, $activity_type)
    {
        $stmt = $this->con->prepare("UPDATE tech_blogs SET 
